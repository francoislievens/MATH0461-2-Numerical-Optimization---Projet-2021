...
println("*             by using the robust variant A               *")
...
for M in [608, 1014, 1521, 3042]
    ...
    # Add constraints:
    @constraint(model, initialA[c=1:M], (sum(mt[c,i]*x[i] for i in 1:N) - m[c])/m[c] <= eps)
    @constraint(model, initialB[c=1:M], (sum(mt[c,i]*x[i] for i in 1:N) - m[c])/m[c] >= eps)
    ...
    if noisy == "Y"
        save(string("Outputs/L1_rob_A_noisy_M", M, "_eps_", eps, ".png"), clamp!(cell, 0.0, 1.0))
        CSV.write(string("csv/L1_rob_A_noisy_M", M, "_eps_", eps, ".csv"), Tables.table(x_hat), writeheader=false)
        println("... Done.")
        println("Image exported in the file Outputs/L1_rob_A_noisy_M", M, "_eps_",eps, ".png")
    else
        save(string("Outputs/L1_rob_A_uncorr_M", M, "_eps_", eps, ".png"), clamp!(cell, 0.0, 1.0))
        CSV.write(string("csv/L1_rob_A_uncorr_M", M, "_eps_", eps, ".csv"), Tables.table(x_hat), writeheader=false)
        println("... Done.")
        println("Image exported in the file Outputs/L1_rob_A_uncorr_M", M, "_eps_",eps, ".png")
    end
end
