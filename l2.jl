...
println("* ======================================================= *")
println("*                                                         *")
println("*              L-2 norm image approximer                  *")
...
@variable(model, t)        # New variable for the epigraph tricks
...
# Export x
x_hat = value.(x)
CSV.write(string("csv/L2_Uncorrupt_M", M, ".csv"), Tables.table(x_hat), writeheader=false)


# Save the image
save(string("Outputs/L2_Uncorrupt_M", M, ".png"), clamp!(cell, 0.0, 1.0))

println("... Done.")
println("Image exported in the file Outputs/L12_Uncorrupt_M", M, ".png")
